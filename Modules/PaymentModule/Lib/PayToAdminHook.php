...
if (!function_exists('pay_to_admin_success')) {
    ...
    function pay_to_admin_success($data): void
    {
        ...
        if ($payable > $receivable && $receivable != 0) {
            //adjust
            withdraw_request_accept_for_adjust_transaction($user->owner?->id, $receivable);
            collect_cash_transaction($provider_id, $receivable);
        }

        //pay to admin
        collect_cash_transaction($provider_id, $amount);
